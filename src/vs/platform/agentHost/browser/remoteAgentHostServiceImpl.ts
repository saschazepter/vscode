...
export class RemoteAgentHostService extends Disposable implements IRemoteAgentHostService {
	...
	get configuredEntries(): readonly IRemoteAgentHostEntry[] {
		// Merge config-based entries with in-memory entries from addRemoteAgentHost
		// (config storage may not work reliably on web)
		const configEntries = this._getConfiguredEntries();
		const result = new Map<string, IRemoteAgentHostEntry>();

		for (const e of configEntries) {
			const addr = normalizeRemoteAgentHostAddress(getEntryAddress(e));
			result.set(addr, {
				name: e.name,
		...
		// Add any in-memory entries not in config
		for (const [address] of this._entries) {
	...
	async addSSHConnection(entry: IRemoteAgentHostEntry, _connection: IAgentConnection): Promise<IRemoteAgentHostConnectionInfo> {
		throw new Error('SSH connections not supported in browser');
	}
	...
	async addRemoteAgentHost(input: IRemoteAgentHostEntry): Promise<IRemoteAgentHostConnectionInfo> {
		...
		const existing = this._getConnectionInfo(address);
		if (existing) {
			return { ...existing, name: input.name };
		}

		// Store name mapping and connect directly (skip config storage
		// because the configuration service may not be ready on web)
		this._names.set(address, input.name);
		...
		try {
			...
			// Config not ready — that's fine, we'll connect directly
		}

		// If reconciliation hasn't connected yet, connect now
		if (!this._entries.has(address)) {
			this._connectTo(address, input.connectionToken);
		}
	...
	private _upsertConfiguredEntry(entry: IRemoteAgentHostEntry): IRemoteAgentHostEntry[] {
		...
		const existingIndex = configuredEntries.findIndex(configuredEntry => normalizeRemoteAgentHostAddress(getEntryAddress(configuredEntry)) === normalizedAddress);
		if (existingIndex === -1) {
