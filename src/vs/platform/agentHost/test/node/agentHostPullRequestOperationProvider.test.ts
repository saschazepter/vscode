...
suite('AgentHostPullRequestOperationContribution', () => {
	...
	function createContribution(status?: IAgentHostPullRequestStatus, isolation?: 'folder' | 'worktree', agentMergeEnabled = false): AgentHostPullRequestOperationContribution {
	...
	test('advertises Create PR and Enable Agent Merge as the last Create PR option when Agent Merge is enabled', () => {
		const provider = createContribution(undefined, undefined, true);
