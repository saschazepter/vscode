...
suite('CopilotAgent', () => {
	...
	suite('worktree announcement', () => {
		...
		test('emits announcement live across an agent restart, before any assistant message exists', async () => {
			// Simulates: worktree session is created (branch metadata persisted),
			// then the agent process restarts before the user has sent the first
			// turn. A new agent instance is constructed pointing at the same DB,
			// and the live announcement must still fire on the very first
			// sendMessage. This is the regression case for the in-memory map
			// the previous design used.
			const sessionId = 'wt-restart-session';
			const session = AgentSession.uri('copilot', sessionId);
			const repositoryRoot = URI.joinPath(URI.file(tmpDir), 'repo');
			await fs.mkdir(repositoryRoot.fsPath, { recursive: true });
			...
			// Phase 1: first agent only persists the branch metadata, then
			// shuts down without ever receiving a sendMessage.
			const firstAgent = createTestAgent(disposables, { sessionDataService, copilotClient: new TestCopilotClient([]), gitService }) as TestableCopilotAgent;
			try {
				await firstAgent.authenticate('https://api.github.com', 'token');
				await firstAgent.resolveWorktreeForTest({
					workingDirectory: repositoryRoot,
					config: { isolation: 'worktree', branch: 'main', branchNameHint: 'restart-test' },
				}, sessionId);
			} finally {
				await disposeAgent(firstAgent);
			}

			// Phase 2: brand-new agent instance, same DB. Without the DB-backed
			// design this test would fail because the in-memory map is empty
			// and no assistant message exists yet for the restore path to
			// touch.
			const expectedBranchName = getCopilotWorktreeBranchName(sessionId, 'restart-test');
			const reopened = createTestAgent(disposables, { sessionDataService, copilotClient: new TestCopilotClient([]), gitService }) as TestableCopilotAgent;
			reopened.registerFakeSession(sessionId, {
				send: async () => { },
				getMessages: async () => [{ session, type: 'message', role: 'user', messageId: 'u1', content: 'hi' }],
				dispose: () => { },
			});

			try {
				await reopened.authenticate('https://api.github.com', 'token');

				const events: IAgentProgressEvent[] = [];
				disposables.add(reopened.onDidSessionProgress(e => events.push(e)));
				await reopened.sendMessage(session, 'hello');

				const deltas = events.filter((e): e is IAgentDeltaEvent => e.type === 'delta');
				assert.strictEqual(deltas.length, 1, 'announcement must still be emitted live on the first sendMessage after a restart');
				assert.ok(deltas[0].content.includes(expectedBranchName), `announcement must contain branch name '${expectedBranchName}', got '${deltas[0].content}'`);

				// And no duplicate emission on subsequent sends.
				events.length = 0;
				await reopened.sendMessage(session, 'follow-up');
				assert.deepStrictEqual(events.filter(e => e.type === 'delta'), [], 'announcement must not be re-emitted on subsequent sends');
			} finally {
				await disposeAgent(reopened);
			}
		});
	});
});
