...
- Expected: materializing a fresh peer after using the default chat succeeds, and the peer request does not contain the default chat's history.
- Observed: after the preceding peer-lifecycle sequence, the peer turn can fail with `sendFailed: Server not found: host`. The same test passes by itself and while recording with a per-test process.
...
- Reproduce:
  ...
  Temporarily enable the Claude variant. Run the whole provider file; a focused run does not reproduce the shared-process symptom.
