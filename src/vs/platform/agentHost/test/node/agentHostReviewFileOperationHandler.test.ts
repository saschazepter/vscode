...
function setup(disposables: Pick<DisposableStore, 'add'>, opts?: { readonly reviewed?: boolean; readonly withWorkingDirectory?: boolean; readonly registerSession?: boolean; readonly baseBranch?: string }): { handler: AgentHostReviewFileOperationHandler; reviewService: TestReviewService; session: URI } {
	...
	return { handler, reviewService, session };
}
...
suite('AgentHostReviewFileOperationHandler', () => {
	...
	test('marks the targeted file as reviewed, passing the resolved base branch', async () => {
		const { handler, reviewService, session } = setup(disposables, { reviewed: true, baseBranch: 'main' });
	...
	test('clears the reviewed mark for the targeted file', async () => {
		const { handler, reviewService, session } = setup(disposables, { reviewed: false });
		const target = URI.file('/repo/src/file.ts');
