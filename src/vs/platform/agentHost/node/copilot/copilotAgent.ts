...
export class CopilotAgent extends Disposable implements IAgent {
	...
	async getSessionMessages(session: URI): Promise<SessionHistoryEvent[]> {
		...
		return [...prependAnnouncementToFirstAssistantMessage(rawMessages, buildWorktreeAnnouncementText(worktreeMeta.branchName))];
	}
	...
	private async _readWorktreeMetadata(session: URI): Promise<{ branchName: string; worktreePath?: URI; repositoryRoot?: URI } | undefined> {
		...
		try {
			const [branchName, worktreePath, repositoryRoot] = await Promise.all([
			...
			return {
				branchName,
				worktreePath: worktreePath ? URI.parse(worktreePath) : undefined,
				repositoryRoot: repositoryRoot ? URI.parse(repositoryRoot) : undefined,
			};
		} finally {
			ref.dispose();
		}
