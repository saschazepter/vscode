...
export interface IAgentService {
	...
	/**
	 * Register that `clientId` is now subscribed to `resource`. The protocol
	 * layer maintains an authoritative server-side refcount so idle session
	 * state can be evicted when no client holds it. Callers should pair every
	 * {@link subscribe} with an `addSubscriber` and a matching
	 * {@link removeSubscriber} when the subscription is released.
	 */
	addSubscriber(resource: URI, clientId: string): void;

	/**
	 * Counterpart to {@link addSubscriber}. Drops `clientId` from the
	 * refcount for `resource`; when the last subscriber is removed, idle
	 * session state for `resource` may be evicted from the server.
	 */
