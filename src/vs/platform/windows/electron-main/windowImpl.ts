...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	load(configuration: INativeWindowConfiguration, options: ILoadOptions = Object.create(null)): void {
		...
		} else if (configuration.isAgentSessionsWindow) {
			windowUrl = FileAccess.asBrowserUri(`vs/code/agentic/electron-browser/workbench/workbench${this.environmentMainService.isBuilt ? '' : '-dev'}.html`).toString(true);
		} else {
