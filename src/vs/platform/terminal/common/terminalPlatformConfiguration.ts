...
const terminalPlatformConfiguration: IConfigurationNode = {
	...
	properties: {
		...
		[TerminalSettingId.AgentHostProfileLinux]: {
			restricted: true,
			markdownDescription: localize('terminal.integrated.agentHostProfile.linux', "The terminal profile to use on Linux for agent host terminals (the shell launched by AI agent tools and remote agent host sessions). Accepts either a profile name from {0} or an inline profile object. When unset, falls back to {1}. Only the executable `path` is honored today; `args` and `env` from the profile are ignored. Applies to the local agent host. Remote agent hosts use their own machine's shell config.", '`#terminal.integrated.profiles.linux#`', '`#terminal.integrated.defaultProfile.linux#`'),
		...
		[TerminalSettingId.AgentHostProfileMacOs]: {
			restricted: true,
			markdownDescription: localize('terminal.integrated.agentHostProfile.osx', "The terminal profile to use on macOS for agent host terminals (the shell launched by AI agent tools and remote agent host sessions). Accepts either a profile name from {0} or an inline profile object. When unset, falls back to {1}. Only the executable `path` is honored today; `args` and `env` from the profile are ignored. Applies to the local agent host. Remote agent hosts use their own machine's shell config.", '`#terminal.integrated.profiles.osx#`', '`#terminal.integrated.defaultProfile.osx#`'),
			type: ['string', 'object', 'null'],
			default: null,
			'anyOf': [
				{ type: 'null' },
		...
		[TerminalSettingId.AgentHostProfileWindows]: {
			restricted: true,
			markdownDescription: localize('terminal.integrated.agentHostProfile.windows', "The terminal profile to use on Windows for agent host terminals (the shell launched by AI agent tools and remote agent host sessions). Accepts either a profile name from {0} or an inline profile object. When unset, falls back to {1}. Only the executable `path` is honored today; `args` and `env` from the profile are ignored. Applies to the local agent host. Remote agent hosts use their own machine's shell config.", '`#terminal.integrated.profiles.windows#`', '`#terminal.integrated.defaultProfile.windows#`'),
			type: ['string', 'object', 'null'],
			default: null,
			'anyOf': [
				{ type: 'null' },
