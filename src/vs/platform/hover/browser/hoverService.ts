...
export class HoverService extends Disposable implements IHoverService {
	...
	showDelayedHover(
		...
		lifecycleOptions: Pick<IHoverLifecycleOptions, 'groupId' | 'delay'>,
		...
		const delay = lifecycleOptions?.delay ?? this._configurationService.getValue<number>('workbench.hover.delay');
		timeout(delay).then(() => {
