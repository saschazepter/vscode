...
export interface ITerminalSandboxEngineHost {
	...
	getWorkspaceStorageReadRoot(): URI | undefined;
	...
	checkSandboxDependencies(forceRefresh: boolean): Promise<ISandboxDependencyStatus | undefined>;
}
...
export class TerminalSandboxEngine extends Disposable {
	...
	async getMissingSandboxDependencies(): Promise<string[]> {
		...
		if (!this._sandboxDependencyStatus || !this._sandboxDependencyStatus.bubblewrapInstalled || !this._sandboxDependencyStatus.socatInstalled) {
			this._sandboxDependencyStatus = await this._host.checkSandboxDependencies(true);
		}
	...
	private async _checkSandboxDependencies(forceRefresh = false): Promise<boolean> {
		...
		const status = await this._host.checkSandboxDependencies(forceRefresh);
	...
	private _getWorkspaceStorageReadPaths(): string[] {
		const root = this._host.getWorkspaceStorageReadRoot();
