...
**Editor-title action visibility.** All single-pane editor-title actions (Maximize/Restore, Toggle Details, Hide Editor, Open in Modal) are hidden while the **editor area is closed** (`MainEditorAreaVisibleContext`). Hide Editor and Toggle Details are additionally shown only when the active tab is **Changes or Files** (`HasDockedDetailsContext`); Hide Editor is further hidden only while the editor area is **not maximized** (`EditorMaximizedContext` negated).
