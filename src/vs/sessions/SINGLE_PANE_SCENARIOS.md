...
A created session opens the side pane to **Editor only** (Changes editor, detail closed) by default; a Changes/file editor becoming active never force-opens the detail (the exceptions are restoring the detail after a transient browser-tab hide and opening the empty **Files placeholder**, which represents the Files view and reveals the Files detail **once** when it becomes active — provided the editor content is visible and no session-switch restore is in progress. The reveal is one-shot: the user can then hide the detail and it stays hidden while the placeholder remains active). A new-session view opens to the **Files detail** (its editor content stays hidden by R1).
...
**Managed Files tab.** The empty Files placeholder tab is shown when the editor area is **closed** or **no real (non-managed) editor is open**. An **auto-ensured** placeholder the user never activated is removed as redundant once a real file/diff is opened into a visible editor area, and re-added when the editor area closes again. But once the user **activates** the Files tab (the `+` **Files** entry, or selecting it), it is **committed** as a normal tab and is no longer auto-removed — it stays open while the user selects or opens other files/tabs, until the user closes it or the session changes.
