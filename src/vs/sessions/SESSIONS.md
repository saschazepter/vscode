...
**Example:** each `NewChatInputWidget` owns a scoped `SessionModelSelectionModel` (`contrib/chat/browser/modelPickerModel.ts`). The model reads `provider.getModels(...)` directly and exposes observable catalog, selection, options, and send eligibility; the desktop and phone pickers consume those observables instead of mirroring them into an `activeSessionHasModels` context key. Menu `when` clauses only gate on genuinely declarative conditions such as phone layout and whether the provider offers a combined config picker.
