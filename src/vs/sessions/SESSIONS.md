...
- **`ISessionsProvider`** (`sessionsProvider.ts`) — Contract every provider implements. Covers workspace discovery, session CRUD, sending requests, model enumeration/selection/presentation (`getModelCatalog`, `getModelPickerOptions`, `onDidChangeModels`, `setModel`), and firing change events.
...
Context keys are an output/gating mechanism, **not** a source of truth. Do **not** mirror dynamic state (e.g. "the active session has models", a count, a selection) into a context key only to read it back in imperative code, and do not call `IContextKeyService.getContextKeyValue(...)` to drive logic. Instead, read state directly from the owning service or observable (`ISessionsService.activeSession`, `ISessionsProvider.getModelCatalog`, etc.) and react with `autorun`/`derived`.
...
**Example:** each `NewChatInputWidget` owns a scoped `SessionModelSelectionModel` (`contrib/chat/browser/modelPickerModel.ts`). The model reads `provider.getModelCatalog(...)` directly and exposes observable catalog, selection, options, and send eligibility; the snapshot's `resolved` bit keeps a restored selection pending until its provider can conclusively report that it is absent. The desktop and phone pickers consume those observables instead of mirroring them into an `activeSessionHasModels` context key. Menu `when` clauses only gate on genuinely declarative conditions such as phone layout and whether the provider offers a combined config picker.

The scoped model applies the pure transitions in `modelPickerSelection.ts`, remembers explicit choices under `sessions.modelPicker.${providerId}.${sessionType}.selectedModelId`, and writes through `ISessionsProvider.setModel`. Omitted `ISessionModelPickerOptions.showAutoModel` is normalized to `true`. The same `hasSelectableModel` observable gates both new-session composer variants, so model-list changes cannot leave their send buttons out of sync.
