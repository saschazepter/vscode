...
Parts manage their own border and background styling via the `updateStyles()` method. In the default light theme, the sessions workbench surface uses the off-white workbench/sidebar background while the card-like chat, auxiliary bar, and panel surfaces use the brighter editor background. Light themes also override the chat, auxiliary bar, and panel card border color in CSS to use `editorWidget.border`, giving those cards a darker outline. Dark and high-contrast mappings continue to use the existing part border tokens. The sessions shell now uses a single root-level gradient layer in `browser/media/style.css` (`.agent-sessions-workbench::before`) that sits behind the workbench parts, blending the shell background with the theme's primary accent signal from `button.background`. The gradient runs from the base shell color at the top-left toward a gentle accent tint in the bottom-right; light and dark themes use different `color-mix(...)` strengths, and high-contrast themes disable the gradient entirely for accessibility. Titlebar/sidebar wrappers are made transparent so that one shared layer reads continuously across the whole window chrome without clipping at part boundaries. These surfaces use a **card appearance** with CSS variables for background and border:
...
| 2026-04-14 | Reworked the sessions accent-tinted background into a single root-level pseudo-element behind the workbench parts, then tuned it to run from a neutral top-left into a bottom-right accent tint, using lighter mixes for light themes, stronger mixes for dark themes, and no gradient in high-contrast themes. |
