...
The Agents window contributes a built-in **Automations** client-tool set with `listAutomations`, `configureAutomation`, and `deleteAutomation`. Listing is read-only and returns stable IDs plus editable fields. Configuration uses the invoking session as the default target for new entries. When the current tool approval policy requires interaction, create/update proposals open the existing dialog for review and delete requests show a **Delete**/**Cancel** confirmation; auto-approved requests are validated and committed directly through `IAutomationService`. Workspace choices in the dialog never update the new-session recent-workspace list. Deletion removes the automation and retained run history while already-dispatched sessions continue. Cancellation, denial, invalid IDs, disabled Automations, and stale reviewed updates leave the ledger unchanged.
