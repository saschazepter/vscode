...
The MCP Servers section combines locally known MCP servers with the effective, exact-name-keyed runtime inventory reported by the active agent-host session (`IAgentHostCustomizationService.getMcpServerInventory(activeSessionResource)`). The raw agent-host customization tree may declare the same runtime name in multiple containers; inventory projection prefers a top-level runtime entry over duplicate children while raw server IDs remain available to protocol and authentication flows. Active-session servers are matched to known workspace, user, extension, plugin, or built-in rows by stable identifiers and display names so the row can show the active session's status, matching `MCP: List Servers`. Active-session servers that do not match any known local/runtime server are appended to the **Workspace** group and counted with the rest of the section.

In the Agents window, workspace-scoped workbench servers are limited to configuration resources under `IAICustomizationWorkspaceService.activeProjectRoot`; user, remote-user, extension, plugin, and built-in rows remain visible. This prevents the internal multi-root workspace from surfacing the same `.vscode/mcp.json` server once for the main checkout and again for each session worktree. Runtime exclusion still uses every workbench-local server ID, so filtered foreign-worktree definitions cannot reappear under **Built-in**. Core VS Code keeps its normal multi-root behavior, and exact same-name rows from distinct visible configuration sources remain separate.
