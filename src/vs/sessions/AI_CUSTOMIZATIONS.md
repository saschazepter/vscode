...
The MCP Servers section combines locally known MCP servers with every MCP customization reported by the active agent-host session (`IAgentHostCustomizationService.getMcpServers(activeSessionResource)`). Exact server name is a collision key, not identity: same-named servers from different user, workspace, plugin, extension, or agent-host sources remain separately represented and configurable. Local/runtime rows retain the priority and enablement chosen by `McpCollisionEnablementModel`, which prefers more specific configuration sources and disables lower-priority collisions without removing them.

Container-backed Agent Host servers carry their parent plugin/directory provenance and effective enablement (`container.enabled && server.enabled`). The list shows that source below the server name, while same-named workbench configurations show their MCP config source. This keeps valid collisions distinguishable and exposes disabled copies instead of collapsing or hiding them. Enabling a child whose container is disabled first enables the container, then the child; disabling a child leaves its container unchanged. Active-session servers are matched to known rows only by stable, unambiguous identifiers or names; unmatched entries remain independent **Workspace** rows so actions cannot target the wrong source.
