...
The Agents window contributes a built-in **Automations** client-tool set with `listAutomations`, `configureAutomation`, and `deleteAutomation`. Listing is read-only and returns stable IDs plus editable fields. Configuration uses the invoking session as the default target for new entries, but never writes directly: it opens the existing create/edit dialog with the proposal prefilled, and only the user's explicit **Create** or **Save** action is committed through `IAutomationService`. Workspace choices in this dialog never update the new-session recent-workspace list. Deletion requires a stable ID and a dedicated **Delete** confirmation that cannot be auto-approved; confirming removes the automation and its retained run history while already-dispatched sessions continue. Cancellation, invalid IDs, disabled Automations, and cancelled tool invocations leave the ledger unchanged.
