...
| `contrib/providers/agentHost/browser/mobile/mobileChatInputConfigPicker.ts` | (standalone) | Phone-only chat-input chip that combines Mode + Model into a unified bottom sheet. It consumes the input-scoped `SessionModelSelectionModel`, so it shares the desktop picker's provider catalog, current selection, canonical persistence, and empty-model state without enumerating language models itself. |
| `contrib/providers/agentHost/browser/mobile/mobileChatPhoneInputPresenter.ts` | `IChatPhonePresenterImpl` | Builds the combined sheet for an opened chat. Agent Host rows come from `ISessionsProvider.getModelCatalog`; sessions-side callers pass their input-scoped context getter, and every action re-resolves that same source, matches provider/session/chat identity through `IUriIdentityService`, then validates the selected id against a fresh provider catalog before writing. |
