...
| `contrib/providers/agentHost/browser/mobile/mobileChatPhoneInputPresenter.ts` | `IChatPhonePresenterImpl` | Builds the combined sheet for an opened chat. Agent Host rows come from `ISessionsProvider.getModels`; every action re-resolves and matches the provider, session, and active-chat identity captured when the sheet opened, then validates the selected id against a fresh provider catalog before writing. |
