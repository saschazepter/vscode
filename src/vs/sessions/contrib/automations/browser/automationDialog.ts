...
class AutomationIsolationGroupActionViewItem extends BaseActionViewItem {
	...
	// Branch picker model observables
	private readonly _branches = observableValue<readonly string[]>('branches', []);
	private readonly _selectedBranch = observableValue<string | undefined>('selectedBranch', undefined);
	private readonly _branchDisabled = observableValue<boolean>('branchDisabled', true);

	// BranchPicker instance
	...
	constructor(
		...
		this._selectedBranch.set(state.branch, undefined);
	}
	...
	override render(container: HTMLElement): void {
		...
		// Branch picker — uses the shared BranchPicker widget
		const branchPickerModel = constObservable<IBranchPickerModel>({
			branches: this._branches,
			selectedBranch: this._selectedBranch,
			disabled: this._branchDisabled,
			setBranch: name => this.onBranchSelected(name),
		});
		this.branchPicker = this.renderDisposables.add(
			this.instantiationService.createInstance(BranchPicker, branchPickerModel)
		);
	...
	// --- Event handlers: delegate to model, then update picker state ---
	...
	private onBranchSelected(branch: string): void {
		...
		this._selectedBranch.set(branch, undefined);
		this.updateState();
	}

	/** Sync checkbox DOM and picker model observables from the IsolationGroupModel. */
	private updateState(): void {
		...
		// Update branch picker disabled state
		this._branchDisabled.set(!this.model.branchPickerEnabled, undefined);
		this._selectedBranch.set(this.state.branch, undefined);
	}

	private observeRepository(folder: URI | undefined): void {
		const myRequestId = ++this.branchRequestId;
		this.branchRepoDisposable.clear();
		this._branches.set([], undefined);
		this._branchDisabled.set(true, undefined);
		...
		void (async () => {
			...
			try {
				...
				this._branches.set(branches, undefined);
			} catch {
				if (myRequestId !== this.branchRequestId) {
					return;
				}
	...
	const chatInputOptions: IChatInputPartOptions = {
		...
		secondaryToolbarActionViewItemProvider: (action, itemOptions) => {
			...
			if (action.id === AUTOMATIONS_ISOLATION_GROUP_ACTION_ID) {
				...
				return new AutomationIsolationGroupActionViewItem(action, state, workspacePicker, instantiationService, gitService, itemOptions);
			}
			return undefined;
		},
	};
