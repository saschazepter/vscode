...
class AdditionalChat extends Disposable {
	...
	constructor(resource: URI, summary: ChatSummary, isNew: boolean = false, parentChat?: URI, sessionIsArchived: IObservable<boolean> = constObservable(false)) {
		super();
...
export class AgentHostSessionAdapter extends Disposable implements ISession {
	...
	private _createAdditionalChat(chatId: string, summary: ChatSummary): AdditionalChat {
		...
		return new AdditionalChat(resource, summary, this._newChatIds.has(chatId), this._resolveParentChatResource(summary.origin), this.isArchived);
	}
