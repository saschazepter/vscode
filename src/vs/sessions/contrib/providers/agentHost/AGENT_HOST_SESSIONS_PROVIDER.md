...
For worktree isolation, first-send materialization creates the worktree through `AgentHostGitService`. Git LFS is optional: when `git-lfs` is not on the agent host's `PATH`, worktree checkout disables the LFS process/smudge filters and leaves pointer files intact instead of failing the send. When `git-lfs` is available, normal LFS checkout behavior is preserved.
