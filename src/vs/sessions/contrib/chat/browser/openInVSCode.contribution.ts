...
import { OpenInActionViewItem } from './openInActionViewItem.js';
import { VSCODE_STABLE_ICON_URI, VSCODE_INSIDERS_ICON_URI } from './openInIcons.js';
import { canOpenInVSCode, canUseLocalPath, createVSCodeOpenUri, OpenInDropdownMenuId, OPEN_IN_TARGET_STORAGE_KEY, OpenInTarget, readStoredOpenInTarget, resolveOpenInTarget } from './openInVSCodeUtils.js';
...
export class OpenInContribution extends Disposable {
	...
	private async _runTarget(target: OpenInTarget, _source: 'primary' | 'menu'): Promise<void> {
		const openTarget = resolveOpenInTarget(
			this.sessionsManagementService,
			this.sessionsProvidersService,
			this.remoteAgentHostService,
		...
		switch (target) {
			...
			case OpenInTarget.VSCodeInsiders: {
				const uri = createVSCodeOpenUri(target, openTarget);
				if (!uri) {
			...
			case OpenInTarget.Finder:
				...
				await this.openerService.open(URI.file(openTarget.filePath), { openExternal: true });
				return;
...
async function runOpenInTarget(accessor: ServicesAccessor, target: OpenInTarget): Promise<void> {
	...
	switch (target) {
		...
		case OpenInTarget.VSCodeInsiders: {
			...
			await openerService.open(uri, { openExternal: true });
			return;
		}
		case OpenInTarget.Finder:
			if (!canUseLocalPath(openTarget)) {
			...
			await openerService.open(URI.file(openTarget.filePath), { openExternal: true });
			return;
		case OpenInTarget.CopyPath:
			if (!canUseLocalPath(openTarget)) {
