...
/**
 * Phone variant of {@link ScopedWorkspacePicker} that renders the picker
 * as a bottom sheet instead of the desktop action-widget popup.
 * Delegates to `super.showPicker()` when the viewport is no longer
 * phone (e.g. user rotated past the phone breakpoint), so a single
 * instance works correctly across rotation.
 */
export class MobileScopedWorkspacePicker extends ScopedWorkspacePicker {
