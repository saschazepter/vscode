...
/**
 * Sessions-only storage type for built-in prompts.
 * Cast as `PromptsStorage` so it flows through the existing pipeline
 * (filtering, grouping) without adding a new enum value to core.
 */
...
export class AgenticPromptsService extends PromptsService {
	 ...
	 * Tagged with {@link BUILTIN_STORAGE} so the sessions UI can
	 * display them in a dedicated "Built-in" group.
	 */
	...
	private async discoverBuiltinPrompts(type: PromptsType): Promise<readonly IPromptPath[]> {
		...
		try {
			const stat = await fileService.resolve(BUILTIN_PROMPTS_URI);
			if (!stat.children) {
				return [];
			}
