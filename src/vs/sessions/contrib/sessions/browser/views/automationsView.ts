...
export class AutomationsCardsWidget extends Disposable {
	...
	constructor(
		...
		this._register(autorun(reader => {
			...
			const unavailableProviderLabels = this.automationService.unavailableProviderLabels.read(reader);
			...
			this.cardsSection.render(items, catalogueState, unavailableProviderLabels, templates);
		}));
...
class AutomationCardsSection extends Disposable {
	...
	private partialUnavailableProviderLabels: readonly string[] = [];
	...
	render(automations: readonly IAutomationDescriptor[], catalogueState: AutomationCatalogueState, unavailableProviderLabels: readonly string[], templates: readonly IAutomationTemplate[]): void {
		...
		this.unavailableDescription.textContent = unavailableProviderLabels.length > 0
			? formatUnavailableAutomationsMessage(unavailableProviderLabels)
		...
		this.renderPartialState(automations.length > 0 ? catalogueState : 'ready', unavailableProviderLabels);
	...
	private renderPartialState(catalogueState: AutomationCatalogueState, unavailableProviderLabels: readonly string[]): void {
		const unavailableProviderLabelsChanged = catalogueState === 'unavailable'
			&& (unavailableProviderLabels.length !== this.partialUnavailableProviderLabels.length
				|| unavailableProviderLabels.some((label, index) => label !== this.partialUnavailableProviderLabels[index]));
		if (this.partialState === catalogueState && !unavailableProviderLabelsChanged) {
			return;
		}
		this.partialState = catalogueState;
		this.partialUnavailableProviderLabels = catalogueState === 'unavailable' ? [...unavailableProviderLabels] : [];
