...
`ProviderAutomationService` keeps the initial aggregate loading until all AfterRestored workbench contributions have completed provider registration. A provider-less window then settles to its legacy-store state, so a legacy-only empty catalogue can be authoritative. After provider settlement, the aggregate reports `error` when any current store fails, otherwise `loading` while any store is loading, and `unavailable` while an unavailable provider has evidence that it owns Automations. Provider-scoped legacy rows and the persisted result of the last authoritative provider catalogue supply that evidence. An unavailable provider with no ownership evidence does not make the aggregate catalogue incomplete.
