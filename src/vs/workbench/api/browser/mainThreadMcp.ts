...
import { IMcpRegistry, IMcpMessageTransport } from '../../contrib/mcp/common/mcpRegistry.js';
	...
	constructor(
		...
		this._register(this._mcpRegistry.registerDelegate({
			canStart(collection, serverDefinition) {
				...
				if (serverDefinition.launch.type === McpServerTransportType.CommandLine && extHostContext.extensionHostKind === ExtensionHostKind.LocalWebWorker) {
					return false;
				}
				return true;
			},
