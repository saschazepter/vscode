...
async function renderChatInput(context: ComponentFixtureContext, fixtureOptions: ChatInputFixtureOptions = {}): Promise<void> {
	...
	const instantiationService = createEditorServices(disposableStore, {
		...
		additionalServices: (reg) => {
			...
			reg.defineInstance(IChatModeService, new class extends mock<IChatModeService>() { override readonly onDidChangeChatModes = Event.None; override getModes(_sessionType: string) { return { builtin: [], custom: [], findModeById: () => undefined, findModeByName: () => undefined }; } }());
