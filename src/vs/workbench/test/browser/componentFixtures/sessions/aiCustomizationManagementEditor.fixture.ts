...
const allStateMcpServers: FixtureAgentHostMcpServer[] = [
	...
	{ ...activeSessionMcpServers[0], id: 'mcp-top-level:fixture:session:Web Search', name: 'Web Search', enabled: false },
];
...
export default defineThemedFixtureGroup({ path: 'chat/aiCustomizations/' }, {
	...
	McpServersTab: defineComponentFixture({
		labels: { kind: 'screenshot', blocksCi: false },
		expectedVisualDescriptions: ['The MCP Servers page shows User, Workspace, and Available tabs with count badges. MCP rows show names and descriptions without configuration file paths.'],
		render: ctx => renderEditor(ctx, {
			sessionResource: localSessionResource,
			selectedSection: AICustomizationManagementSection.McpServers,
		}),
