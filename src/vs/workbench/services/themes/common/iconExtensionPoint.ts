			...
			for (const extension of delta.added) {
				...
				for (const id in extensionValue) {
					...
					if (typeof defaultIcon === 'string') {
						...
						collector.error(nls.localize('invalid.icons.default', "'configuration.icons.default' must be either a reference to the id of a theme icon (string) or a icon definition (object) with properties `fontPath` and `fontCharacter`."));
					}
				}
			}
