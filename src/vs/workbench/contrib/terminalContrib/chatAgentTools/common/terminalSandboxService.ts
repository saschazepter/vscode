...
export class TerminalSandboxService extends Disposable implements ITerminalSandboxService {
	...
	constructor(
		...
		const host: ITerminalSandboxEngineHost = {
			...
			checkSandboxDependencies: (forceRefresh: boolean) => this._resolveSandboxDependencyStatus(forceRefresh),
		};
	...
	private _resolveWorkspaceStorageReadRoot(): URI | undefined {
	...
	private async _resolveSandboxDependencyStatus(forceRefresh: boolean): Promise<ISandboxDependencyStatus | undefined> {
		// forceRefresh: the engine handles caching of the resolved status; this hook
		// is only invoked when a fresh probe is desired, so we always re-query.
		void forceRefresh;
