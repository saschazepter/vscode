...
suite('HooksExecutionService', () => {
	...
	suite('executeHook', () => {
		...
		test('executes hook commands via proxy and returns semantic results', async () => {
			...
			const results = await service.executeHook(HookType.PreToolUse, sessionUri, 'test-input');
		...
		test('passes cancellation token to proxy', async () => {
			...
			await service.executeHook(HookType.PreToolUse, sessionUri, undefined, cts.token);
		...
		test('passes input to proxy', async () => {
			...
			await service.executeHook(HookType.PreToolUse, sessionUri, testInput);
