...
suite('VoiceSessionController', () => {
	...
	function createController(voiceClientService: TestVoiceClientService, chatService: IChatService = new TestChatService(), micCaptureService?: IMicCaptureService): IVoiceSessionController {
	...
	test('auto-listen opens a passive mic turn so the backend does not latch user_is_speaking', () => {
		...
		const controller = createController(voiceClientService, undefined, mic);
