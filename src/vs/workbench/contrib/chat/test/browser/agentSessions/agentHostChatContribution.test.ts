...
suite('AgentHostChatContribution', () => {
	...
	suite('subagent grouping', () => {
		...
		test('parent subagent tool created in PendingConfirmation state still gets subagent toolSpecificData', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			// Repro for the duplicate-container bug: when the parent task tool is
			// briefly in PendingConfirmation before `tool_ready` (with confirmed)
			// auto-approves it, the live ChatToolInvocation is created without
			// subagent toolSpecificData. The renderer then mounts it as a regular
			// tool invocation widget ("Used 'task'"), and when inner child tool
			// calls arrive with subAgentInvocationId set, they create a separate
			...
			// 1. Parent task tool starts (Streaming).
			fire({
				type: 'session/toolCallStart', session, turnId,
				toolCallId: parentToolCallId, toolName: 'task', displayName: 'Task',
				_meta: { subagentDescription: 'do work', subagentAgentName: 'helper' },
			} as ISessionAction);

			// 2. toolCallReady WITHOUT confirmed → tool sits in PendingConfirmation.
			fire({
				type: 'session/toolCallReady', session, turnId,
				toolCallId: parentToolCallId,
				invocationMessage: 'Spawning subagent',
				confirmationTitle: 'Run subagent?',
			} as ISessionAction);

			// Let the throttler flush — the parent invocation gets created here,
			// in PendingConfirmation state.
			await timeout(50);

			// 3. NOW the agent host auto-approves and the tool transitions to Running.
			fire({
				type: 'session/toolCallReady', session, turnId,
				toolCallId: parentToolCallId,
				invocationMessage: 'Spawning subagent',
			...
			// 4. Child session emits an inner tool call.
			const childTurnId = 'child-turn-1';
			const childToolCallId = 'tc-child-1';
			const fireChild = (action: ISessionAction) => {
				agentHostService.fireAction({ action, serverSeq: 1000, origin: undefined });
			...
			fireChild({
				type: 'session/turnStarted', session: childSessionUri, turnId: childTurnId,
				userMessage: { text: '' },
			} as ISessionAction);
			fireChild({
				type: 'session/toolCallStart', session: childSessionUri, turnId: childTurnId,
			...
			assert.ok(parent, 'parent task tool invocation should be emitted');
			// THE KEY ASSERTION: even though the parent invocation was first
			// created during PendingConfirmation, by the time the inner tool
			// calls arrive its toolSpecificData must be subagent-kind so the
			// renderer groups everything into a single subagent container.
			assert.strictEqual(parent!.toolSpecificData?.kind, 'subagent', 'parent must end up as subagent kind even when first observed in PendingConfirmation');

			assert.ok(child, 'child tool invocation should be forwarded');
			assert.strictEqual(child!.subAgentInvocationId, parentToolCallId, 'child should be tagged for grouping');
		}));
