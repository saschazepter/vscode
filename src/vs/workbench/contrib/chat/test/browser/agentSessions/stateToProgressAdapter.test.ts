		...
		test('uses shell_exit exit code for completed SDK shell tool history', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					...
					{ type: ToolResultContentType.ShellExit, shellId: '104', exitCode: 127, cwd: '/repo', outputPreview: 'preview only\n', outputTruncated: true },
				],
				success: true,
			});
		...
		test('strips legacy shell completion marker from terminal fallback output', () => {
			const tc = createCompletedToolCall({
				...
				content: [
					...
					{ type: ToolResultContentType.ShellExit, shellId: '104', exitCode: 127, cwd: '/repo' },
				],
				success: true,
			});
