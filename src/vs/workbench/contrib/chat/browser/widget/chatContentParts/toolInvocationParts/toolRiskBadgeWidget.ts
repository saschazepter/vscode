...
export class ToolRiskBadgeWidget extends Disposable {
	...
	setAssessment(assessment: IToolRiskAssessment): void {
		// Green / clear commands: nothing useful to say — hide the row entirely.
		if (assessment.risk === ToolRiskLevel.Green && assessment.needsExplanation === false) {
			this.setHidden();
			return;
		}
		switch (assessment.risk) {
			case ToolRiskLevel.Green:
				// Green / unclear: render the explanation neutrally — no warning icon, no color.
