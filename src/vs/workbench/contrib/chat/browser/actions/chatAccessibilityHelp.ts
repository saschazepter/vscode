...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat' | 'agentView' | 'quickChat' | 'editsView' | 'agentView', keybindingService: IKeybindingService): string {
	...
	if (type === 'panelChat' || type === 'quickChat' || type === 'agentView') {
		...
		content.push(localize('chat.inspectResponse', 'In the input box, inspect the last response in the accessible view{0}. Thinking content is included in order when enabled.', '<keybinding:editor.action.accessibleView>'));
