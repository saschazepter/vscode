...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat' | 'quickChat' | 'editsView' | 'agentView', keybindingService: IKeybindingService, supportsFileReferences: boolean): string {
	...
	if (type === 'panelChat' || type === 'quickChat' || type === 'agentView') {
		...
		content.push(localize('chat.vscodePet', 'Type /vscode-pet to show or hide the VS Code pet above the input. When it is visible, use Tab to focus it and press Enter or Space to show it some love.'));
