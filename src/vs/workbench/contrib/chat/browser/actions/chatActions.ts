...
export function registerChatActions() {
	...
	// and keep the config submenu in the overflow ("...") area so other actions remain discoverable
	...
	MenuRegistry.appendMenuItem(MenuId.ViewTitle, {
		...
		group: '3_configure',
		when: ContextKeyExpr.and(ContextKeyExpr.equals('view', ChatViewId), ContextKeyExpr.has(`config.${ChatConfiguration.ChatCustomizationMenuEnabled}`)),
		order: 0
	});

	// When customizations menu is disabled, show the legacy gear submenu in navigation
	MenuRegistry.appendMenuItem(MenuId.ViewTitle, {
		submenu: CHAT_CONFIG_MENU_ID,
		title: localize2('config.label.legacy', "Configure Chat"),
		group: 'navigation',
