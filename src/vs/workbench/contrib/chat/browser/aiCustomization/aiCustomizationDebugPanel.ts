 ...
 *   1. Provider output — what `provideChatSessionCustomizations()` returns
 *   2. Raw PromptsService data — lower-level service output (when using promptsService adapter)
...
export async function generateCustomizationDebugReport(
	...
	// Stage 1: Provider output — every harness goes through a provider now
	const activeProvider = activeDescriptor?.itemProvider ?? promptsServiceItemProvider;
	const isAdapterProvider = !activeDescriptor?.itemProvider || activeDescriptor.itemProvider === promptsServiceItemProvider;
	if (activeProvider) {
		const providerLabel = isAdapterProvider ? 'PromptsService Adapter' : 'Extension Provider';
		await appendProviderData(lines, activeProvider, promptType, providerLabel);
	}

	// Stage 2: Raw PromptsService data — useful for diagnosing the adapter
	if (isAdapterProvider) {
