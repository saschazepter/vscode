...
/**
 * Unified item source that fetches items from a single
 * {@link ICustomizationItemProvider} (extension-contributed, the
 * promptsService adapter, or a remote agent harness) and normalizes
 * them into list items.
 *
 * Harnesses that need to surface both local and remote customizations
 * (e.g. an AHP harness) compose them inside their item provider so this
 * class never has to special-case the source.
 */
