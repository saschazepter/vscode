...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.ProgressBorder]: {
			type: 'boolean',
			default: product.quality !== 'stable',
			markdownDescription: nls.localize('chat.progressBorder.enabled', "Show an animated gradient border around the chat input while the agent is working or thinking. When enabled, this overrides {0} to be off. Has no effect when reduced motion is enabled.", '`#chat.persistentProgress.enabled#`'),
		},
