...
export class ChatSessionsService extends Disposable implements IChatSessionsService {
	...
	public getCheckpointHandler(sessionResource: URI): ((requestId: string, token: CancellationToken) => Promise<void>) | undefined {
		const sessionData = this._sessions.get(sessionResource);
		return sessionData?.session.checkpointHandler;
	}
