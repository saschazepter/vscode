...
export namespace ChatContextKeys {
	...
	export const chatSessionHasPermissionPicker = new RawContextKey<boolean>('chatSessionHasPermissionPicker', false, { type: 'boolean', description: localize('chatSessionHasPermissionPicker', "True when the active chat session contributes its own items into the chat permission picker. Indicates contribution, not visibility \u2014 the picker may still be hidden by other `when` clauses.") });
