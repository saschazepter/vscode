		...
		const files = await Promise.all(allAgentFiles.map(async (promptPath): Promise<IAgentDiscoveryResult> => {
			...
			try {
				...
				const when = isExtensionPromptPath(promptPath) && promptPath.when
					...
					: undefined;
				if (!ast.header) {
					const agent: ICustomAgent = { uri, name, agentInstructions, source, target, contentHash, visibility: { userInvocable: true, agentInvocable: true }, ...(when !== undefined ? { when } : undefined) };
