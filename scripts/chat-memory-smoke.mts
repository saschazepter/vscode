 ...
 *
 * Intended workflow:
 * - Run `node scripts/chat-memory-smoke.mts --iterations 3 --no-heap-snapshots` for a fast health check.
 * - Run `node scripts/chat-memory-smoke.mts --iterations 8 --heap-snapshot-label 03-iteration-01 --heap-snapshot-label 03-iteration-08` when comparing post-warmup heap snapshots.
...
function printHelp(): void {
	...
	console.log([
		'Usage: node scripts/chat-memory-smoke.mts [options]',
		'',
		...
		'',
		'Example:',
		`  node scripts/chat-memory-smoke.mts --iterations 5 --output ${tmp}`,
		'',
		'Watch/login/retry example:',
		'  node scripts/chat-memory-smoke.mts --keep-open --iterations 1 --no-heap-snapshots',
	].join('\n'));
}
