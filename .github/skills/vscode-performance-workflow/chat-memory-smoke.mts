 ...
 *
 * Intended workflow:
 * - Run `node .github/skills/vscode-performance-workflow/chat-memory-smoke.mts --iterations 3 --no-heap-snapshots` for a fast health check.
 * - Run `node .github/skills/vscode-performance-workflow/chat-memory-smoke.mts --iterations 8 --heap-snapshot-label 03-iteration-01 --heap-snapshot-label 03-iteration-08` when comparing post-warmup heap snapshots.
 * - Inspect the output folder's summary.json, screenshots, and optional heap/*.heapsnapshot files.
 *
 * The default profile is persistent at .build/chat-memory-smoke/user-data so auth can be reused across runs.
 * Pass --temporary-user-data when a clean, disposable profile is required.
 */
...
async function main(): Promise<void> {
	...
	const generatedUserDataDir = options.temporaryUserData ? path.join(outputDir, 'user-data') : path.join(root, '.build', 'chat-memory-smoke', 'user-data');
	...
	const ownsUserDataDir = ownsCode && options.temporaryUserData && !options.keepOpen && !options.keepUserData && options.userDataDir === undefined;
...
function printHelp(): void {
	...
	console.log([
		'Usage: node .github/skills/vscode-performance-workflow/chat-memory-smoke.mts [options]',
		'',
		...
		'',
		'Example:',
		`  node .github/skills/vscode-performance-workflow/chat-memory-smoke.mts --iterations 5 --output ${tmp}`,
		'',
		'Watch/login/retry example:',
		'  node .github/skills/vscode-performance-workflow/chat-memory-smoke.mts --keep-open --iterations 1 --no-heap-snapshots',
	].join('\n'));
}
