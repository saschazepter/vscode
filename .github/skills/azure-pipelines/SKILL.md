...
If you prefer using `az` directly:
...
az pipelines build list \
  ...
  --definition-ids 111 \
  --top 10 \
  --query "[].{id:id, status:status, result:result, branch:sourceBranch, started:startTime}"
```
...
### Get build logs
```bash
az pipelines build logs list \
  --organization https://dev.azure.com/monacotools \
  --project Monaco \
  --build-id <build-id>

# Download specific log
az pipelines build logs download \
  --organization https://dev.azure.com/monacotools \
  --project Monaco \
  --build-id <build-id> \
  --log-id <log-id>
...
az rest --method patch \
  --url "https://dev.azure.com/monacotools/Monaco/_apis/build/builds/<build-id>?api-version=7.0" \
