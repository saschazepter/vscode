...
- For custom clickable elements (e.g. picker triggers, title bar pills, or other `<div>`/`<span>` elements styled as buttons) that open pickers or menus on click, listen to **both** `EventType.CLICK` and `TouchEventType.Tap` (and call `Gesture.addTarget` on the element). Using only `CLICK` is insufficient on iOS where tap events don't synthesize a click. The base `Button` class already does this correctly, so this rule applies to custom non-`<button>` trigger elements.
