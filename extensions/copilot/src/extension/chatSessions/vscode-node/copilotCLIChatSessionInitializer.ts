...
export interface ICopilotCLIChatSessionInitializer {
	...
	): Promise<{ session: IReference<ICopilotCLISession> | undefined; isNewSession: boolean; model: string | undefined; agent: SweCustomAgent | undefined; trusted: boolean }>;
	...
	createDelegatedSession(
		...
		token: vscode.CancellationToken
	): Promise<{ session: IReference<ICopilotCLISession>; model: string | undefined; agent: SweCustomAgent | undefined }>;
}
...
export class CopilotCLIChatSessionInitializer implements ICopilotCLIChatSessionInitializer {
	...
	async getOrCreateSession(
		...
		const [{ workspaceInfo, cancelled, trusted }, model, agent] = await Promise.all([
			...
			this.resolveModelId(request, token),
	...
	async createDelegatedSession(
		...
		const session = await this.sessionService.createSession({ workspace, agent, model, mcpServerMappings: options.mcpServerMappings }, token);
	 ...
	 */
	async resolveModelId(request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<string | undefined> {
