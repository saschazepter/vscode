...
describe('ChatSessionInitializer', () => {
	...
	describe('resolveModelId', () => {
		it('returns resolved model from request.model.id', async () => {
			...
			const result = await initializer.resolveModelId(request, CancellationToken.None);
		...
		it('returns default model when request is undefined', async () => {
			const { initializer } = createInitializer();
			const result = await initializer.resolveModelId(undefined, CancellationToken.None);
			expect(result).toBe('default-model');
		});
