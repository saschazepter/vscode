  ...
  begin
    ...
    begin
      ...
      if not SessionEndFileExists() and not CancelFileExists() then begin
        ...
        Exec(ExpandConstant('{app}\{#VersionedResourcesFolder}\tools\inno_updater.exe'), ExpandConstant('"{app}\{#ExeBasename}.exe" ' + BoolToStr(LockFileExists()) + ' "{cm:UpdatingVisualStudioCode}" "' + ExpandConstant('{param:cancel}') + '"'), '', SW_SHOW, ewWaitUntilTerminated, UpdateResultCode);
        DeleteFile(ExpandConstant('{app}\updating_version'));
