...
import { getCopilotExcludeFilter, prepareBuiltInCopilotExtensionShims } from './lib/copilot.ts';
...
function prepareCopilotShimsTaskREH(platform: string, arch: string, destinationFolderName: string) {
	return async () => {
	...
	BUILD_TARGETS.forEach(buildTarget => {
		...
		['', 'min'].forEach(minified => {
			...
			const packageTasks: task.Task[] = [
				...
				prepareCopilotShimsTaskREH(platform, arch, destinationFolderName)
			];
